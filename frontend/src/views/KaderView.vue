...
const grouped = computed(() =>
	positions.map((pos) => ({
		...pos,
		players: players.value.filter((p) => p.position === pos.key)
	}))
);
...
async function remove(p: Player) {
	if (!window.confirm(`${p.name} wirklich aus dem Kader löschen? Strafen und Rückmeldungen werden mit entfernt.`)) return;
...
<template>
	<div class="page-head">
		<h1>Kader</h1>
		<button class="btn primary sm" @click="openCreate"><Plus :size="14" aria-hidden="true" /> Spieler</button>
	</div>
				...
				<table class="tbl">
					<tbody>
						<tr v-for="p in group.players" :key="p.id">
							<td class="num" style="width: 40px; color: var(--gold)">{{ p.number ?? '–' }}</td>
							<td>{{ p.name }}</td>
							<td style="width: 90px"><span class="chip" :class="p.status">{{ p.status }}</span></td>
							<td style="width: 80px; text-align: right">
								<button class="btn sm" aria-label="Bearbeiten" @click="openEdit(p)"><Pencil :size="12" /></button>
								<button class="btn sm danger" aria-label="Löschen" @click="remove(p)"><Trash2 :size="12" /></button>
							</td>
						</tr>
					</tbody>
				</table>
	...
	<AppModal v-if="showForm" :title="editId ? 'Spieler bearbeiten' : 'Neuer Spieler'" @close="showForm = false">
		<form @submit.prevent="submit">
			...
			<div class="grid cols-2">
				<div class="field">
					<label for="pl-number">Rückennummer</label>
					<input id="pl-number" v-model="form.number" type="number" min="1" max="99" />
				</div>
				<div class="field">
